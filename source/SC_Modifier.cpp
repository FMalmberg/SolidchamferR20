...
public:
	...
	static NodeData *Alloc(void) { return NewObjClear(SC_Modifier); }
};
...
BaseObject* SC_Modifier::GetVirtualObjects(BaseObject* op, HierarchyHelp* hh){
	...
	Int32 mode=data->GetInt32(SOLIDCHAMFER_EDGESELECTION);
	...
	case SC_PHONG:
		...
		if(tag){
			...
			float angle=tagdata->GetFloat(PHONGTAG_PHONG_ANGLE);
	...
	long subdivision=data->GetInt32(SOLIDCHAMFER_SUBDIVISION);
	double Radius=data->GetFloat(SOLIDCHAMFER_RADIUS);
...
{	
	...
	data->SetFloat(SOLIDCHAMFER_RADIUS,5.0);
	data->SetInt32(SOLIDCHAMFER_SUBDIVISION,0);
	data->SetInt32(SOLIDCHAMFER_EDGESELECTION,SC_LIVE);
...
{
	return RegisterObjectPlugin(ID_SC_MODIFIER,String("SolidChamfer Modifier"),OBJECT_GENERATOR|OBJECT_INPUT,SC_Modifier::Alloc,String("osolidchamfer"),AutoBitmap(String("SolidChamferModifier.tif")),0);
}

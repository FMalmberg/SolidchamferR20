...
Int32  SC_SubdivisionData::GetNewEdgeVertex(Int32 a, Int32 b, map<SC_Edge,Int32, SC_Edge> & edgemap, SC_Offsetdata& offset, SC_EditMesh & m){
	Int32 newVertexIndex;
	Vector origin, dir;
	map<SC_Edge,Int32, SC_Edge>::iterator it=edgemap.find(SC_Edge(a,b));
	if(it!=edgemap.end()){
...
void SC_SubdivisionData::Subdivide(SC_Offsetdata& offset){
	...
	map<Int32,VectorAccumulator> newOffsets;
	SC_EditMesh m(mesh);
	int nstrips=strips.size();
	int nfaces=faces.size();
	Int32 newVertexIndex1, newVertexIndex2,newVertexIndex3, newVertexIndex4,newCenterVertexIndex, newFaceIndex;
	Int32 a,b,c,d;
	CPolygon* polys=mesh->GetPolygonW();
	Vector origin, dir;
	map<SC_Edge, Int32, SC_Edge> edgemap;
	map<Int32,Int32> centermap;
	...
	Int32 newFaceIndex1, newFaceIndex2,newFaceIndex3;
	...
	map<Int32,VectorAccumulator>::iterator it;
	...
	for(it2=OpenEdges.begin(); it2!=OpenEdges.end(); it2++){
		SC_Edge edge=*it2;
		Int32 midpoint= GetNewEdgeVertex(edge.GetFirst(), edge.GetSecond(), edgemap, offset,  m);
...
void SC_SubdivisionData::AddStripBoundary(Int32 vertex_id){
	StripBoundary.insert(vertex_id);
}

void SC_SubdivisionData::AddFaceBoundary(Int32 vertex_id){
	FaceBoundary.insert(vertex_id);
}

bool SC_SubdivisionData::IsStripBoundary(Int32 vertex_id){
	return (StripBoundary.find(vertex_id)!=StripBoundary.end());
}

bool SC_SubdivisionData::IsFaceBoundary(Int32 vertex_id){

...
void FixSelection(BaseSelect* selectedEdges, PolygonObject* obj, Neighbor* n){
	set<Int32> newElements;

	CPolygon* polys=obj->GetPolygonW();

	Int32 verts[4];
	Int32 pind, eind, v1, v2, nsides, pind2, eind2, elem;
	Int32 seg=0,a,b,i;
	while (selectedEdges->GetRange(seg++, LIMIT<Int32>::MAX, &a,&b)){
		for (i=a; i<=b; ++i){
...
Vector Merge(Vector OldV, Vector NewV){
	return (OldV+NewV)/(1+Dot(!OldV,!NewV));
}


void TraverseAdjacent(Int32 active_poly, 
	Int32 center_vertex,
	Int32 active_vertex, 
	SC_Edgemap& edges, 
	Neighbor& neighbors, 
	...
	CPolygon* polys){
		...
		Int32 last_active_vertex=active_vertex;
		Int32 new_edge_vertex=edges(center_vertex,active_vertex).GetNew(center_vertex);
...
void SolidChamfer(PolygonObject* mesh, BaseSelect* edgeSelection, SC_Offsetdata& offset, SC_SubdivisionData& sd,CORNER_STYLE cs){
	...
	map<Int32,SC_Facedata> faces;
	map<Int32,SC_Vertexdata> vertices;
	SC_Edgemap edges;
	set<SC_Edge,SC_Edge> selectedEdges;
	set<Int32> new_edges;
	set<SC_Edge,SC_Edge> openEdges;
	map<Int32,Int32> openFaceEdges;
	...
	Int32 verts[4];
	Int32 seg=0,a,b,i;
	Int32 pind,eind,v1,v2,nsides;
	while (edgeSelection->GetRange(seg++, LIMIT<Int32>::MAX, &a,&b)){
		for (i=a; i<=b; ++i){
	...
	map<Int32,SC_Facedata>::iterator it2;

	Int32 new_face_verts[4];
	bool prev_edge_sel, next_edge_sel;
	Vector p_cur, p_next, p_prev;
	Int32 i_cur, i_next, i_prev;
	Int32 next_edge_index;
	Int32 prev_edge_index;
	...
	map<Int32,Int32>::iterator iter2;
	for(iter2=openFaceEdges.begin(); iter2!=openFaceEdges.end(); iter2++){

		Int32 pind=iter2->first/8;
		Int32 eind=(iter2->first%8)/2;
		...
		Int32 p1=verts[eind];
		Int32 p2=verts[GetNext(eind,nsides)];

		Int32 adjacent=p1;
		if((iter2->first%8)%2==0){adjacent=p2;}

		Int32 new_p=iter2->second;
